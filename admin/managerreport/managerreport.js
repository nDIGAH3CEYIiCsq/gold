...
function managerreport_init()
{
	...
	$(".edit_order").live("click", function()
	{
		...
		td_price.html("<input type='text' name='price' value='" + price + "' class='edit_order_price' /><div class='old'>" + price + "</div>");
	...
	$(".save_order").live("click", function()
	{
		...
		var price = tr.find("td:eq(4) INPUT").val();
		...
		var xml = content_get("edit_order", {'id': order_id, 'price': price, 'payment_method': payment_method, 'delivery_method': delivery_method, 'delivery_price': delivery_price, 'status': status, 'name': name, 'phone': phone, 'email': email, 'address': address});
		if (xml.find("errors"))
		{
	...
	$(".save_item").live("click", function()
	{
		...
		var code = tr.find("td:eq(1) INPUT").val();
		var model = tr.find("td:eq(2) INPUT").val();
		var price_base = tr.find("td:eq(3) INPUT").val();
		var price = tr.find("td:eq(4) INPUT").val();
		var size = tr.find("td:eq(5) INPUT").val();
		var count = tr.find("td:eq(6) INPUT").val();
		var status = tr.find("td:eq(7) SELECT").val();

		var xml = content_get("edit_item", {'id': id, 'code': code, 'name': model, 'initial_price': price_base, 'price': price, 'size': size, 'count': count, 'status_manager': status});
		if (xml.find("errors"))
		{
			var errors = xml.find("errors").text();
			if (errors != "")
		...
		$(xml.find("item").text()).insertAfter(tr);
		tr.remove();
	})
